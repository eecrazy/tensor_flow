...
label_size=len(string.ascii_lowercase) + 1
...
class BatchGenerator(object):
  ...
  def _next_batch(self):
    ...
    train = np.zeros(shape=(self._batch_size), dtype=np.int32)
    label= np.zeros(shape=(self._batch_size), dtype=np.int32)
    
    for b in range(self._batch_size):
      train[b] = bi2id(self._text[self._cursor[b]:self._cursor[b]+2])
...
# trains,labels=train_batches.next()
# print(batches2string(trains,labels))
# trains,labels=train_batches.next()
# print(batches2string(trains,labels))

# trains,labels=valid_batches.next()
# print(batches2string(trains,labels))
# print(batches2string(trains,labels))
...
def sample(prediction):
  ...
  p = np.zeros(shape=[1, label_size], dtype=np.float)
...
def random_distribution():
  ...
  b = np.random.uniform(0.0, 1.0, size=[1, label_size])
  return b/np.sum(b, 1)[:,None]


def one_hot_represent(num,train_labels):
  temp = np.zeros(shape=(num,label_size), dtype=np.float32)
  return_data=[]
...
with lstm_graph.as_default():
  ...
  train_labels = tf.placeholder(tf.float32, shape=[num_unrollings,batch_size,label_size])
  # print(tf.get_shape(train_labels))
...
with tf.Session(graph=lstm_graph) as session:
  ...
  for step in range(num_steps):
    trains,labels = train_batches.next() #10*64,10*64
    train_labels_one_not = one_hot_represent(batch_size,labels)

    feed_dict = dict()
    for i in range(num_unrollings):
      feed_dict[train_data[i]] = trains[i]
    ...
    if step % summary_frequency == 0:
      ...
      for _ in range(valid_size): #64
        valid,valid_labels = valid_batches.next()
        predictions = sample_prediction.eval({sample_input: valid[0]})
        valid_logprob = valid_logprob + logprob(predictions, one_hot_represent(1,valid_labels))
      ...
      if step % (summary_frequency) == 0:
        ...
        for _ in range(5):
          ...
          for _ in range(70):
            feed=sentence[-2:]
            prediction = sample_prediction.eval({sample_input: [bi2id(feed)]})
            feed = sample(prediction)
            sentence += id2char(np.argmax(feed))
