...
with lstm_graph.as_default():
  ...
  # Optimizer.优化器，也看不太懂？exponential_decay这个函数好好看看？
  global_step = tf.Variable(0)
  learning_rate = tf.train.exponential_decay(1.0, global_step, 1000, 0.9, staircase=True)
  ...
  for step in range(num_steps):
    ...
    if step % summary_frequency == 0:
      ...
      if step % (summary_frequency) == 0:
        ...
        print('=' * 80)
        for _ in range(5):  #生成5行每行80个字母
          feed = sample(random_distribution())  #生成一个随机的1-hot字母表示
          sentence = characters(feed)[0] #第一个种子字母
          reset_sample_state.run()
          for _ in range(79):
            prediction = sample_prediction.eval({sample_input: feed}) #根据feed预测下一个字母的概率分布
            prediction = sample(prediction)  #这里加入了随机性
...
# 
# 我们想要训练一个bigram LSTM，但这会带来输入的稀疏性问题，计算消耗大。
# 
# a- Introduce an embedding lookup on the inputs, and feed the embeddings to the LSTM cell instead of the inputs themselves. 
# 引入一个embedding lookup table，把这个embedding输入到LSTM单元中，而不是每一个输入。
# 
# b- Write a bigram-based LSTM, modeled on the character LSTM above. 写一个bigram-LSTM。
# 
# c- Introduce Dropout. For best practices on how to use Dropout in LSTMs, refer to this [article](http://arxiv.org/abs/1409.2329). 引入Dropout。可以参考这篇文章。
# 
# ---
...
# 参见课程lecture看看如何构建一个seq2seq模型，参考这篇文章来学习最佳实践。
# 
# ---
